...
def read_yaml(path_to_yaml: Path):
    ...
    Raises:
        custom exception if error rises
    Returns:
        Config file content
    """
    ...
    # Read the config file to gather settings for the rest of the execution
    return content
...
if __name__ == "__main__":
    ...
    param = read_yaml("prompt.yaml") 
    
    meta_list = [ 

        # - - - - - - - - GLIGEN on text grounding for inpainting - - - - - - - - # 
        dict(
            ckpt = "gligen_checkpoints/inpainting_pytorch_model.bin",
            input_image = param["image"],
            prompt = param["prompt"],
            phrases = param["phrase"],
            locations = param["location"], # mask will be derived from box 
            save_folder_name=param["output_folder"]
        )]
...
"""    
    meta_list = [ 
        ...
        dict(
            ckpt = "GLIGEN/gligen_checkpoints/diffusion_pytorch_model.bin",
